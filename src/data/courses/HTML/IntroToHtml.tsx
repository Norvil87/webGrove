...
export const introToHtml: ICourseLesson = {
  ...
  <pre><code><span class="tag"><<span>p</span>></span>Завтра начну изучать вэб-разработку!<span class="tag"><<span>/p</span>></span></code></pre>
         ...
         <p>Текст, который находится между открывающим и закрывающим тэгом называется контентом или содержимым HTML элемента.</p>
         <p>Кстати, <code>p</code> - это не что иное как параграф (<i>paragraph</i>), и служит он для того же, для чего параграф в обычной книге или статье - для отображения небольшого объема логически связанного текста.</p>`,
      goal: `Добавьте на страницу еще один HTML элемент.`,
      initValues: {
...
<pre><code><span class="tag"><<span>main</span>></span>
  <span class="tag"><<span>div</span>></span>
    <span class="tag"><<span>p</span>></span>Этот параграф является потомком элемента div и имеет соседа справа - элемент span<span class="tag"><<span>/p</span>></span>
    <span class="tag"><<span>span</span>></span>Элемент span также является потомком div и имеет соседа слева - элемент p<span class="tag"><<span>/span</span>></span>
  <span class="tag"><<span>/div</span>></span>
<span class="tag"><<span>/main</span>></span></code></pre>
  ...
  <div>
...

    {
      ...
      url: "headers",
      theory: `<p>Заголовки в HTML служат той же цели, что и заголовки в любых других средствах массовой информации - привлечь внимание читателя и кратко описать контент. Заголовки, как правило, выделяются бОльшим размером шрифта, чем основной контент, и жирным начертанием.</p>
...
<pre><code><span class="tag"><<span>h2</span>></span>Каталог товаров<span class="tag"><<span>/h2</span>></span></code></pre>`,
      goal: `Добавьте на страницу еще несколько заголовков.`,
      initValues: {
        html: `<h1>Москва</h1>
<h2>Культура</h2>
<h3>Музеи</h3>
...
<pre><code><span class="tag"><<span>div</span>></span>
  <span class="tag"><<span>p</span>></span>В этом параграфе одно <span class="tag"><<span>span</span>></span>слово<span class="tag"><<span>/span</span>></span> выделено с помощью span<span class="tag"><<span>/p</span>></span>
<span class="tag"><<span>/div</span>></span></code></pre>
...
<div>
  ...
  <h3>Кинотеатры</h3>
    ...
    {
      id: 7,
      header: "Смысловое выделение текста - тэги em и strong",
      url: "html-highlight",
...
<pre><code><span class="tag"><<span>p</span>></span> В Москве <span class="tag"><<span>strong</span>></span>огромное<span class="tag"><<span>/strong</span>></span> количество музеев, но среди них выделяются настоящие гиганты исторических, технических и художественных экспозиций, которые необходимо посетить <span class="tag"><<span>em</span>></span>хотя бы один раз в жизни<span class="tag"><<span>/em</span>></span>.<span class="tag"><<span>/p</span>></span></code></pre>
...
<pre><code><span class="tag"><<span>p</span>></span>Друг мне в подарок прислал<span class="tag"><<span>br</span>></span>Рису, а я его пригласил<span class="tag"><<span>br</span>></span>В гости к самой луне. <span class="tag"><<span>hr</span>></span> Глубокою стариной<span class="tag"><<span>br</span>></span>Повеяло… Сад возле храма<span class="tag"><<span>br</span>></span> Засыпан палым листом.<span class="tag"><<span>/p</span>></span></code></pre>
...
<div>
  ...
  <h3>Кинотеатры</h3>
    ...
    {
      id: 9,
      header: "Отображение неупорядоченных списков - тэги ul и li",
      url: "unordered-lists",
      theory: `<p>Есть много информации, которую удобно отображать в виде списка (например, список покупок). Рассмотрим, как отображить список с помощью элемента <code>ul</code>.</p>
        <p><code>ul</code> отобразит <em>неупорядоченный</em> список элементов, каждый из них по умолчанию будет помечен круглым маркером. Однако нельзя просто так вставить текст в <code>ul</code>. Каждый элемент списка должен быть обернут в элемент <code>li</code>. Например, вот так:</p>
...
<div>
  ...
  <h3>Кинотеатры</h3>
    ...
    {
      id: 10,
      header: "Отображение упорядоченных списков - тэг ol",
      url: "ordered-lists",
